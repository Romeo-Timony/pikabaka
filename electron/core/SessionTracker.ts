...
export class SessionTracker {
    ...
    getFormattedContext(lastSeconds: number = 120): string {
        ...
        // Include in-progress interviewer speech so "What to say" works before a final arrives
        const interim = this.lastInterimInterviewer?.text?.trim();
        if (interim) {
            const alreadyCovered = items.some(
                (item) =>
                    item.role === 'interviewer' &&
                    (item.text.includes(interim) || interim.includes(item.text))
            );
            if (!alreadyCovered) {
                lines.push(`[INTERVIEWER (LIVE)]: ${interim}`);
            }
        }
