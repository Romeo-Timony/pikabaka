...
export class WindowHelper {
  ...
  /** Meeting overlay default width: half the current display work area. */
  private getOverlayDefaultWidth(): number {
    const workArea = screen.getPrimaryDisplay().workArea
    return Math.max(380, Math.floor(workArea.width * 0.5))
  }
  ...
  /** Home/launcher size — matches the reference launcher screenshot (~1024×796). */
  private getLauncherDefaultBounds(): { x: number; y: number; width: number; height: number } {
    const workArea = screen.getPrimaryDisplay().workArea
    const width = Math.min(1024, Math.max(880, Math.floor(workArea.width * 0.7)))
    const height = Math.min(796, Math.max(640, Math.floor(workArea.height * 0.92)))
    const x = Math.round(workArea.x + (workArea.width - width) / 2)
    const y = Math.round(workArea.y + Math.max(16, Math.floor(workArea.height * 0.04)))
  ...
  public createWindow(): void {
    ...
    const { x, y, width, height } = this.getLauncherDefaultBounds()
    ...
    const overlayDefaultWidth = this.getOverlayDefaultWidth()
  ...
  public syncOverlayInteractionPolicy(): void {
    ...
    if (passthrough) {
      // Full click-through. Do NOT use { forward: true } + hover re-enable: on Windows
      // turning ignore off for a button makes the entire HWND eat clicks again, so
      // desktop/folders under the overlay stop working. Interact via global hotkeys.
      this.overlayWindow.setIgnoreMouseEvents(true);
      this.overlayWindow.setFocusable(false);
      console.log('[WindowHelper] Overlay mouse passthrough ON (full click-through)');
    } else {
  ...
  /**
   * Kept for compatibility; while passthrough is on we always stay click-through.
   */
  public setOverlayClickThrough(ignore: boolean): void {
    if (!this.overlayWindow || this.overlayWindow.isDestroyed()) return;
    ...
    // Ignore requests to capture mouse during passthrough — that regresses desktop clicks.
    void ignore;
    this.overlayWindow.setIgnoreMouseEvents(true);
  }
  ...
  public switchToOverlay(inactive?: boolean): void {
    ...
    if (this.overlayWindow && !this.overlayWindow.isDestroyed()) {
      ...
      const targetHeight = Math.max(currentBounds.height, Math.floor(workArea.height * 0.7), 216);
      const overlayWidth = this.getOverlayDefaultWidth();
  ...
  public switchToLauncher(inactive?: boolean): void {
    ...
    // Show Launcher FIRST
    if (this.launcherWindow && !this.launcherWindow.isDestroyed()) {
      // Restore the home-screen size (don't keep a shrunk overlay-era size)
      if (!this.launcherWindow.isMaximized()) {
        this.launcherWindow.setBounds(this.getLauncherDefaultBounds())
      }
      if (process.platform === 'win32' && this.contentProtection) {
