...
import { QA_INTERVIEW_OS_PROMPT } from "../llm/prompts"
...
export function registerCoreHandlers(appState: AppState): void {
  ...
  safeHandle("gemini-chat-stream", async (event, message: string, imagePaths?: string[], context?: string, options?: { skipSystemPrompt?: boolean }) => {
    try {
      ...
      // UI quick-action prompts are not real spoken turns — do not pollute the meeting transcript
      const isUiTriggerPrompt =
        /^what should i say/i.test(trimmedMessage) ||
        trimmedMessage.startsWith('You are a real-time interview assistant') ||
        trimmedMessage.startsWith('CONTEXT:');

      if (!isUiTriggerPrompt && trimmedMessage) {
        intelligenceManager.addTranscript({
          text: trimmedMessage,
          speaker: 'user',
          timestamp: Date.now(),
      ...
      // Always merge live STT transcript. Frontend "conversationContext" is only chat UI
      // history and previously skipped this injection whenever it was non-empty.
      try {
        const liveContext = intelligenceManager.getFormattedContext(180);
        if (liveContext && liveContext.trim().length > 0) {
          const chatHistory = (context || '').trim();
          context = chatHistory
            ? `--- LIVE MEETING TRANSCRIPT ---\n${liveContext}\n\n--- CHAT HISTORY ---\n${chatHistory}`
            : liveContext;
          console.log(`[IPC] Injected live transcript (${liveContext.length} chars) into gemini-chat-stream`);
        } else {
          console.warn('[IPC] No live transcript available for gemini-chat-stream');
        }
      } catch (ctxErr) {
        console.warn("[IPC] Failed to inject live transcript:", ctxErr);
      }

      try {
        // USE streamChat which handles routing.
        // Default to the Interview OS (QA prompt) for optimal spoken answers.
        const stream = llmHelper.streamChat(
          message,
          imagePaths,
          context,
          options?.skipSystemPrompt ? "" : QA_INTERVIEW_OS_PROMPT
        );
  ...
  // Temporary click-through override while passthrough mode remains enabled.
  // Used by the overlay renderer to capture clicks over UI (buttons, chat, transcript).
  safeHandle("set-overlay-click-through", async (_, ignore: boolean) => {
    appState.getWindowHelper().setOverlayClickThrough(!!ignore)
    return { success: true }
  })

  // Renderer may request ignore-mouse updates; while passthrough is on we stay
  // fully click-through (partial capture breaks desktop clicks under the HWND).
  ipcMain.on('set-ignore-mouse-events', (event, ignore: boolean, _options?: Electron.IgnoreMouseEventsOptions) => {
    const win = BrowserWindow.fromWebContents(event.sender)
    if (!win || win.isDestroyed()) return
    if (!appState.getOverlayMousePassthrough()) {
