...
export class EmbeddingPipeline {
    ...
    private async _doInitialize(config: AppAPIConfig): Promise<void> {
        ...
        try {
            ...
            // Auto-reindex meetings whose embeddings were built with another provider/dimension
            const incompatibleCount = this.vectorStore.getIncompatibleMeetingsCount(this.provider.name);
            if (incompatibleCount > 0) {
                const stateRow = this.db.prepare("SELECT value FROM app_state WHERE key = 'last_embedding_provider'").get() as any;
                const lastProvider = stateRow?.value || 'unknown';
                console.log(
                    `[EmbeddingPipeline] Found ${incompatibleCount} incompatible meetings` +
                    ` (was ${lastProvider}, now ${this.provider.name}); auto-reindexing...`
                );
                void this.reindexIncompatibleMeetings().catch((err) => {
                    console.error('[EmbeddingPipeline] Auto-reindex failed:', err);
                });
            }
    ...
    /**
     * Delete embeddings that used a different provider/dimension and requeue them.
     */
    async reindexIncompatibleMeetings(): Promise<void> {
        const providerName = this.provider?.name;
        if (!providerName) {
            console.error('[EmbeddingPipeline] Cannot re-index: no active embedding provider.');
            return;
        }

        const count = this.vectorStore.getIncompatibleMeetingsCount(providerName);
        if (count === 0) {
            console.log('[EmbeddingPipeline] No incompatible meetings found to reindex.');
            return;
        }

        console.log(`[EmbeddingPipeline] Re-indexing ${count} incompatible meetings for ${providerName}...`);
        ...
        for (const meetingId of affectedMeetingIds) {
            await this.queueMeeting(meetingId);
        }

        console.log(`[EmbeddingPipeline] Successfully requeued ${affectedMeetingIds.length} meetings for re-embedding.`);
    }
