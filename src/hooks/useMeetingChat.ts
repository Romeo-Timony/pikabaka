...
export function useMeetingChat() {
  ...
  // Build conversationContext from chat UI only (live STT is injected in main process)
  const allMessagesForContext = useMemo(() => {
    const fromChat: Message[] = chatMessages.map((m) => ({
      id: m.id,
      // Assistant replies are NOT the interviewer — keep them as system/assistant
  ...
  const handleWhatToSay = useCallback(async () => {
    ...
    await submitPrompt({
      ...
      // Always use live STT context via gemini-chat-stream (not JIT RAG)
      skipRag: true,
    });
